Feature: Functional: Ensure Handoff functionality is correctly implemented for the Chat channel

  Background:
    * url 'https://hub.enableriq-qa.com/experience/v1/support/handoff'
    * header Authorization = 'Bearer eyJraWQiOiJyMzlzUGlydDhFdllJbXY3YkE2eDdlb1l6N1RoUFpTQ0lLeHNqQmJReW5jPSIsImFsZyI6IlJTMjU2In0..Y_1hCbsj-ReWQRayVsE6cYST5zU1lsmK2pVeGSJWn9yMJdziMsOWyLlfCgQP-FCu0Dkg0ivkLNK-34DwyCdghHDYcFs6t-GcCc0lqf0b9mnEOYExk6wszcrWAuSS9VfmuOhnJg4n3za-qen3GXD74WXE6-QJWCq1OmGmwNp8l6VLVG0mfdLeVPVYlJXKFN5h4y1Dar7NfUWcvgmH-2VAACgfSWu-9myPaxh2sDqAY6Ggih5yrjyHbtVYZt6M-8TjJCp06EY4BhHjQnp0ZADrUvd55QZ_UN5OP6huDVm-70CeB7lC5QBTrOBM3USu82iPhpzjnDUOZmgsQ8LsQ8QPBg'

  Scenario: Functional: Ensure Handoff functionality is correctly implemented for the Chat channel
    # step: Send POST /api/chat/handoff with body {handoff: true, chatId: '12345'}
    # step: Verify HTTP response status code is 200
    # step: Check the response body for the expected JSON structure
    Given path '/api/chat/handoff'
    And request {"handoff": true, "chatId": "12345"}
    When method POST
    Then status 200
