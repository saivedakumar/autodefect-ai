Feature: Negative/Boundary: Handle edge cases such as invalid input parameters or network issues

  Background:
    * url 'https://hub.enableriq-qa.com/experience/v1/support/handoff'
  ...
  Scenario: Negative/Boundary: Handle edge cases such as invalid input parameters or network issues
    # step: Send POST /api/chat/handoff with body {handoff: true, chatId: '12345', invalidField: 'value'}
    # step: Verify HTTP response status code is 400 (Bad Request)
    # step: Check the response body for error messages related to invalid input parameters or network issues
    Given path '/api/chat/handoff'
    And request {"handoff": true, "chatId": "12345", "invalidField": ""}
    When method POST
    Then status 200
